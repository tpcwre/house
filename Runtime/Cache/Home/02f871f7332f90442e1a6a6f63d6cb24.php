...
<div style='font:normal normal 12px normal;padding-left:15px;margin-bottom:80px'>
	...
	<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div style='border-bottom:1px solid #ccc;height:80px;margin-top:25px;'>	<!--信息展示-->
			...
			<div style='height:50px;width:75%;border:0px solid' class='float'>
				...
				<div style='padding:3 0 0 30px;' >
					<?php echo ($vo["fbsj"]); ?>
					<a href='/house/index.php/Home/Index/moreinfo?id=<?php echo ($vo["id"]); ?>' style='text-decoration:none'>　更多信息...</a>
				</div>
			</div>
	...
	<?php $__FOR_START_7997__=$page2['pmin'];$__FOR_END_7997__=$page2['pmax'];for($k=$__FOR_START_7997__;$k <= $__FOR_END_7997__;$k+=1){ if($page2['pnow'] == $k): ?><div style='display:inline-block;margin:3px;border:1px solid blue;padding:3px'>
...
<?php $__FOR_START_4914__=$page2['pmin'];$__FOR_END_4914__=$page2['pmax'];for($k=$__FOR_START_4914__;$k <= $__FOR_END_4914__;$k+=1){ if($page2['pnow'] == $k): ?><div style='display:inline-block;margin:3px;border:1px solid blue;padding:3px'>
			<a style='text-decoration:none;color:#000' href='<?php echo ($_SERVER['PHP_SELF']); ?>?pnow=<?php echo ($k); ?>'><?php echo ($k); ?></a>
		</div>	
	<?php else: ?>
		<div style='display:inline-block;margin:3px'>
